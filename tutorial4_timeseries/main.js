 ...
 * LOAD DATA
 * */
d3.csv("../data/SuicideAroundtheworld.csv", d => ({
  year: new Date(d.Year, 0, 1),
  country: d.Entity,
  suicide: +d.suicides_no,
})).then(raw_data => {
...
function init() {
  ...
  yScale = d3
    .scaleLinear()
    .domain([0, d3.max(state.data, d => d.suicide)])
...
function draw() {
  ...
  yScale.domain([0, d3.max(filteredData, d => d.suicide)]);
    ...
    .call(
      selection =>
        selection
          ...
          .attr("cy", d => yScale(d.suicide)) // started from the bottom, now we're here
    );
